...
class SmallStraight extends Rule {
  ...
  evalRoll = dice => {
    ...
    if (diceSet.has(2) && diceSet.has(3) && diceSet.has(4) && (diceSet.has(1) || diceSet.has(5)))
      return this.score;

    if (diceSet.has(3) && diceSet.has(4) && diceSet.has(5) && (diceSet.has(2) || diceSet.has(6)))
      return this.score;
...
const yahtzee = new Yahtzee({ score: 50, description: "50 points for Yahtzee(all dice are the same)" });
